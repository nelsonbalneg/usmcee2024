...
{{-- 
                ...
                <div class="text-sm text-slate-500">
                    <p class="flex items-center px-2.5 py-0.5 text-xs font-medium rounded border bg-slate-100 border-transparent text-slate-500 dark:bg-slate-500/20 dark:text-zink-200 dark:border-transparent">{{ Auth::user()->firstname }}
                        {{ Auth::user()->middlename ? Str::substr(Auth::user()->middlename, 0, 1) . '.' : '' }}
                        {{ Auth::user()->lastname }} {{ Auth::user()->suffix }}</p>
                        <p id="current-datetime" class="ml-2 text-xs"></p>
                ...
                <div class="flex gap-3 ms-auto">
                    ...
                    <div class="relative flex items-center dropdown h-header">
                        ...
                        <div class="absolute z-50 hidden p-4 ltr:text-left rtl:text-right bg-white rounded-md shadow-md !top-4 dropdown-menu min-w-[14rem] dark:bg-zink-600"
                            ...
                            <h6 class="mb-2 text-sm font-normal text-slate-500 dark:text-zink-300">Welcome to USMCEE
                            </h6>
                            <a href="#!" class="flex gap-3 mb-3">
                                ...
                                <div>
                                    <h6 class="mb-1 text-15"> {{ Auth::user()->firstname }} {{ Auth::user()->lastname }}
                                        {{ Auth::user()->middlename }} {{ Auth::user()->suffix }}</h6>
                                    <p class="text-slate-500 dark:text-zink-300"> CEE Applicant</p>
                                </div>
                            </a>
                            <ul>
                                    ...
                                    <li class="pt-2 mt-2 border-t border-slate-200 dark:border-zink-500">
                                        <a class="block ltr:pr-4 rtl:pl-4 py-1.5 text-base font-medium transition-all duration-200 ease-linear text-slate-600 dropdown-item hover:text-custom-500 focus:text-custom-500 dark:text-zink-200 dark:hover:text-custom-500 dark:focus:text-custom-500"
                                            ...
                                            onclick="event.preventDefault(); this.closest('form').submit();"><i
                                                data-lucide="log-out" class="inline-block size-4 ltr:mr-2 rtl:ml-2"></i>
                                            Sign Out</a>
                                    </li>
                                </form>
                            </ul>
